...
(function() {
  ...
  LAppModel.prototype.draw = function() {
    ...
    for (var i = 0; i < count; i++) {
      ...
      // Convert Cubism pixel coords to NDC
      // Cubism: origin top-left, x right, y down
      ...
      for (var v = 0; v < vertexCount; v++) {
        ...
        var ndcX = (cx / this._canvasWidth) * 2 - 1;
        var ndcY = 1 - (cy / this._canvasHeight) * 2;

        // Apply aspect ratio correction - fit model in viewport maintaining aspect
        if (canvasAspect > 1) {
          // Wide canvas - scale X
          ndcX = ndcX / canvasAspect;
        } else {
          // Tall canvas - scale Y
          ndcY = ndcY / canvasAspect * canvasAspect;
        }
