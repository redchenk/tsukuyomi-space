...
async function main() {
    ...
    const content = `moderation-check-${Date.now()}`;
    const createRes = await postJson('/api/messages', { content }, userToken);
    assert.equal(createRes.response.status, 201);
    assert.equal(createRes.body.data.status, 'pending');
    assert.match(createRes.body.message, /审核/);

    const hiddenRes = await request('/api/messages/plaza/test-before');
    assert.equal(hiddenRes.response.status, 200);
    assert.equal(hiddenRes.body.data.some(item => item.id === createRes.body.data.id), false);

    const adminListRes = await request('/api/admin/messages', {
        headers: jsonHeaders(adminToken)
    });
    assert.equal(adminListRes.response.status, 200);
    const pending = adminListRes.body.data.find(item => item.id === createRes.body.data.id);
    assert.ok(pending);
    assert.equal(pending.status, 'pending');

    const approveRes = await postJson(`/api/admin/messages/${createRes.body.data.id}/approve`, {}, adminToken);
    ...
    const visible = visibleRes.body.data.find(item => item.id === createRes.body.data.id);
    assert.ok(visible);
